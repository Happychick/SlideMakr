...
def db_set_error(error_hash: str, error_data: dict):
    ...
    try:
        ...
        """, (error_hash, error_data['error_code'],
              error_data.get('correct_code'), error_data['error_msg'],
              error_data['count']))
        conn.commit()
        return True
    except Exception as e:
...
def db_get_error(error_hash: str) -> dict:
    ...
    try:
        ...
        cur.execute("SELECT * FROM slide_errors WHERE error_hash = %s",
                    (error_hash, ))
        result = cur.fetchone()
...
def create_presentation(code_client, credentials, instructions_text,
    ...
    Analyze the user's instructions and return a valid JSON response with this          format, where you decided on a title and whether to use a template or not:
    {
    ...
    TITLE EXTRACTION:
    - Extract a clear, concise presentation title from the instructions text
...
class ErrorDB:
    ...
    def _get_structure_hash(self, code_dict: Dict) -> str:
        ...
        def normalize(obj):
            if isinstance(obj, dict):
                return {
                    k:
                    normalize(v) if k
                    in ['slideLayoutReference', 'pageProperties'] else "VALUE"
        ...
        return hashlib.md5(json.dumps(normalized,
        ...
        if not is_valid:
            ...
            try:
                ...
                fixes_applied.append(
                    f"Fixed request {i}: Applied known correction")
            except json.JSONDecodeError:
                fixed_requests.append(req)
                fixes_applied.append(
                    f"Request {i}: Correction failed to parse")
        else:
            fixed_requests.append(req)
